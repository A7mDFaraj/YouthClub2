...
export default function Hero5() {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 flex-1 flex items-end md:items-center justify-center md:justify-start px-0 md:px-4 sm:px-8 lg:px-16 w-full pt-[60vw] sm:pt-[45vw] md:pt-20 pb-0">
        <motion.div 
          initial={{ opacity: 0, y: 50, scale: 0.95 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
          className="relative bg-[#0A1A2F]/60 md:bg-white/10 backdrop-blur-3xl md:backdrop-blur-2xl border-t border-white/20 md:border-x md:border-b p-8 pb-12 sm:p-8 md:p-14 lg:p-16 rounded-t-[2.5rem] rounded-b-none md:rounded-[2.5rem] shadow-[0_-20px_50px_-15px_rgba(0,0,0,0.5)] md:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] flex flex-col items-center gap-6 md:gap-10 max-w-full md:max-w-lg w-full mx-auto md:mx-0"
        >
          {/* Subtle inner glow */}
          <div className="absolute inset-0 bg-gradient-to-br from-white/20 via-transparent to-transparent rounded-t-[2.5rem] rounded-b-none md:rounded-[2.5rem] pointer-events-none" />
          ...
          >
            {/* Beige background glow behind logo to make it obvious */}
            <div className="absolute inset-0 bg-[#ffefd0] blur-2xl md:blur-3xl opacity-40 rounded-full w-full h-full scale-125 z-0" />
            ...
            <Image 
              ...
              width={300} 
              height={300} 
              className="w-48 sm:w-56 md:w-[300px] h-auto object-contain drop-shadow-[0_0_25px_rgba(255,255,255,0.4)] relative z-10"
              priority
            />
          </motion.div>
          ...
          >
            <h1 className="text-white text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold text-center leading-[1.4] drop-shadow-lg font-sans">
              أكثر من <span className="text-[#ffefd0] bg-white/10 px-3 md:px-4 py-0.5 md:py-1 rounded-xl md:rounded-2xl backdrop-blur-sm border border-white/20 inline-block mx-1 shadow-[0_0_20px_rgba(255,239,208,0.3)]">210</span> نادي شبابي في مكان واحد
            </h1>
          </motion.div>
